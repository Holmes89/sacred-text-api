...
@Component
@Scope("prototype")
public class SearchBibleTextAndVerseCommand extends BaseCommand<List<VerseDTO>>{
	...
	protected SearchBibleTextAndVerseCommand() {
		super(Setter.withGroupKey(HystrixCommandGroupKey.Factory.asKey("SacredTextAPI"))
				.andCommandKey(HystrixCommandKey.Factory.asKey("SearchBibleTextAndVerse")));
	}
	
	
	public SearchBibleTextAndVerseCommand setTerm(String term){
		this.term=term;
		return this;
	}
	...
	protected List<VerseDTO> run() throws Exception {
		try{
			return searchService.searchBibleVerseAndText(term);
		}
