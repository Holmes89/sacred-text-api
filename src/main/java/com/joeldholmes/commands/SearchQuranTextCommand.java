...
@Component
@Scope("prototype")
public class SearchQuranTextCommand extends BaseCommand<List<VerseDTO>> {
	...
	protected SearchQuranTextCommand() {
		super(Setter.withGroupKey(HystrixCommandGroupKey.Factory.asKey("SacredTextAPI"))
				.andCommandKey(HystrixCommandKey.Factory.asKey("SearchQuranText")));
	}
	
	
	public SearchQuranTextCommand setTerm(String term){
		this.term=term;
		return this;
	}
	...
	protected List<VerseDTO> run() throws Exception {
		try{
			return searchService.searchQuranText(term);
		}
