...
public class SearchQuranTextCommandTest {

	@Mock
	SearchService searchService;
	
	@InjectMocks
	private SearchQuranTextCommand cmd = new SearchQuranTextCommand();
	...
	public void testCommand() throws ServiceException{
		...
		Mockito.when(searchService.searchQuranText(Mockito.anyString())).thenReturn(dtos);
	...
	public void testException() throws ServiceException{
		...
		Mockito.when(searchService.searchQuranText(Mockito.anyString())).thenThrow(new ServiceException(ErrorCodes.NULL_INPUT, "Chapter cannot be null"));
